export function formatAccNo(accNo) {
...
export function timeFilters() {
  const d = new Date() // val 为表格内取到的后台时间
  const month =
...
export function getMonthFirst() {
  ...
  const month =
      d.getMonth() + 1 < 10 ? '0' + (d.getMonth() + 1) : d.getMonth() + 1
  var times = d.getFullYear() + '-' + month + '-' + '01'
  return times
}
export function filterTimes(val) {
  if (val == null || val === '') {
    return null
  } else {
...
export function getMonday() {
  ...
  var mondayDate = m_year + '-' + m_month + '-' + m_date
...
export function getStartAndEndDate(days) {
  var now = new Date()
  var nowAfterDays = now.setDate(now.getDate() - days)
  nowAfterDays = new Date(nowAfterDays)
  var endTime = nowAfterDays.getFullYear() + '-' + (nowAfterDays.getMonth() + 1) + '-' + nowAfterDays.getDate()
  return endTime
}
